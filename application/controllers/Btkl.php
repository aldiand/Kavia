...
class Btkl extends AUTH_Controller {
    	...
    	$this->form_validation->set_rules('id_pegawai', 'Pegawai', 'trim|required');
  		...
  		if ($this->form_validation->run() == TRUE) {
  			...
  			if ($result > 0) {
					...
					$this->M_report->insert_jurnal(511, $result, 'd', ($harga));
					$this->M_report->insert_jurnal(111, $result, 'c', ($harga));
  ...
  public function prosesUpdate() {
    ...
    $this->form_validation->set_rules('jam_keluar', 'jam keluar', 'trim|required');
        ...
        $this->M_report->insert_jurnal(513, $this->input->post('id'), 'd', ($total_jam*$harga));
        $this->M_report->insert_jurnal(511, $this->input->post('id'), 'c', ($total_jam*$harga));
				$out['status'] = '';
