    ...
    $this->load->view('bbTerpakai/list_data', $data);
      ...
      $jumlah = $this->M_bahanBaku->get_stok_by_id($data['id_bbb']);
  		...
  		if ($this->form_validation->run() == TRUE) {
  			...
  			if ($result > 0) {
          $result2 = $this->M_bahanBaku->pakaiStok($data['id_bbb'], $data['jumlah']);
          if ($result2 > 0){
						$harga = $this->M_bahanBaku->get_harga_by_id($data['id_bbb']);
						$this->M_report->insert_jurnal(512, $result, 'd', ($data['jumlah']*$harga));
						$this->M_report->insert_jurnal(112, $result, 'c', ($data['jumlah']*$harga));
    				$out['status'] = '';
